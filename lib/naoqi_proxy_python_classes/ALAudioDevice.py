#!/usr/bin/env python
# Class autogenerated from .\alaudiodeviceproxy.h
...
class ALAudioDevice(object):
    ...
    def defaultInput(self):
        ...
        :returns qi::uint32_t: The default input index
        """
        if not self.proxy:
            self.proxy = self.session.service("ALAudioDevice")
        return self.proxy.defaultInput()

    def defaultOutput(self):
        """return the default output

        :returns qi::uint32_t: The default output index
        """
        if not self.proxy:
            self.proxy = self.session.service("ALAudioDevice")
        return self.proxy.defaultOutput()
    ...
    def input(self, index):
        ...
        :param qi::uint32_t index: The input index
        :returns AL::ALValue: An AudioDeviceInfo
        """
        if not self.proxy:
            self.proxy = self.session.service("ALAudioDevice")
        return self.proxy.input(index)
    ...
    def listInputs(self):
        ...
        :returns AL::ALValue: A list of AudioDeviceInfo
        """
        if not self.proxy:
            self.proxy = self.session.service("ALAudioDevice")
        return self.proxy.listInputs()
    ...
    def output(self, index):
        ...
        :param qi::uint32_t index: The output index
        :returns AL::ALValue: An AudioDeviceInfo
        """
        if not self.proxy:
            self.proxy = self.session.service("ALAudioDevice")
        return self.proxy.output(index)
    ...
    def setDefaultInput(self, index):
        ...
        if not self.proxy:
            self.proxy = self.session.service("ALAudioDevice")
        return self.proxy.setDefaultInput(index)

    def setDefaultOutput(self, index):
